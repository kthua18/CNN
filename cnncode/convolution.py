...
with open("prob_map.txt") as ff:
    probVals = np.array([float(line.strip()) for line in ff])
...
for img in range(0,1):
    ...
    for i in range(0, 48):
        new2 = []
        for j in range(0, 40): 
            new2.append(probVals[countf])
            countf += 1
        new1.append(new2)
    finalprob.append(new1)
