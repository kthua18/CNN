...
void single_layer(struct CNN *param,
	...
	single_layer_1:for (int i = 0; i < IN_R * IN_C; i++) {
		test_output[0][0][i] = input[i];
	}

	lyr:for (i = 0; i < 5; i++) {
		img1:for(int r = 0; r < IN_R; r++) {
			img2: for (int c = 0; c < IN_C; c++) {
				kernel:for (int y = 0; y < 5; y++) {
					y_coor = r + y;
					x_coor = c - 2;
					if ((x_coor >= 0 && x_coor < IN_C) && (y_coor >= 0 && y_coor < IN_R)) {
						temp1 = param->weights[0][0][i][((y + pad_zeros) * kernel_x) + (-2 + pad_zeros)] *
								test_output[0][0][IN_C * y_coor + x_coor];
					}

					x_coor = c - 1;
					if ((x_coor >= 0 && x_coor < IN_C) && (y_coor >= 0 && y_coor < IN_R)) {
						temp2 = param->weights[0][0][i][((y + pad_zeros) * kernel_x) + (-1 + pad_zeros)] *
								test_output[0][0][IN_C * y_coor + x_coor];
					}

					x_coor = c;
					if ((x_coor >= 0 && x_coor < IN_C) && (y_coor >= 0 && y_coor < IN_R)) {
						temp3 = param->weights[0][0][i][((y + pad_zeros) * kernel_x) + (pad_zeros)] *
								test_output[0][0][IN_C * y_coor + x_coor];
					}

					x_coor = c + 1;
					if ((x_coor >= 0 && x_coor < IN_C) && (y_coor >= 0 && y_coor < IN_R)) {
						temp4 = param->weights[0][0][i][((y + pad_zeros) * kernel_x) + (1 + pad_zeros)] *
								test_output[0][0][IN_C * y_coor + x_coor];
					}

					x_coor = c + 2;
					if ((x_coor >= 0 && x_coor < IN_C) && (y_coor >= 0 && y_coor < IN_R)) {
						temp5 = param->weights[0][0][i][((y + pad_zeros) * kernel_x) + (2 + pad_zeros)] *
								test_output[0][0][IN_C * y_coor + x_coor];
					}

					test_output[1][0][IN_C * r + c] += temp1 + temp2 + temp3 + temp4 + temp5;
				}
			}
		}
	}
